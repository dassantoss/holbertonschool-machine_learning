...
def create_masks(inputs, target):
    ...
    Returns:
    - encoder_mask: Padding mask for the encoder `(batch_size, 1, 1,
    seq_len_in)`.
    - combined_mask: Combined look-ahead and padding mask for the first
      attention block in the decoder `(batch_size, 1, seq_len_out,
      seq_len_out)`.
    - decoder_mask: Padding mask for the second attention block in the
      decoder `(batch_size, 1, 1, seq_len_in)`.
    """
    # Create padding masks for the inputs and targets
    encoder_mask = create_padding_mask(inputs)
    decoder_mask = create_padding_mask(inputs)

    # Create a look-ahead mask for the target to prevent attending to future
    # tokens
    look_ahead_mask = create_look_ahead_mask(tf.shape(target)[1])

    # Create a padding mask for the target
    dec_target_padding_mask = create_padding_mask(target)

    # Combine the look-ahead mask and the target padding mask
