...
class DeepNeuralNetwork:
    ...
    def train(self, X, Y, iterations=5000, alpha=0.05):
        ...
        Args:
            X (numpy.ndarray): Numpy array with shape (nx,m) containing
            the input data.
            Y (numpy.ndarray): Numpy array with shape (1,m) containing
            the correct labels.
            iterations (int): Number of iterations to train over.
            alpha (float): Learning rate.

        Raises:
            TypeError: If `iterations` is not an integer or `alpha` is
                not a float.
            ValueError: If `iterations` is not positive or `alpha` is
                not positive.

        Returns:
            tuple: A tuple containing the predictions after training
